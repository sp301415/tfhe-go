...
// Most of the implementation are copied from Go 1.20 implementation of gaussian sampling.
// See: src/math/rand/normal.go
...
// GaussianSampler is a struct for sampling from discrete gaussian distribution.
// This uses Ziggurat Method.
type GaussianSampler[T constraints.Integer] struct {
	StdDev float64

	baseInt32Sampler UniformSampler[int32]
}

// uniformFloat64 returns a float64 between 0 and 1.
// Used in NormFloat64.
func (s GaussianSampler[T]) uniformFloat64() float64 {
	var x uint64
	binary.Read(s.baseInt32Sampler, binary.BigEndian, &x)

	return float64(x&0x1fffffffffffff) / float64(0x1fffffffffffff)
}

// normalFloat64 returns a normally distributed float64 in
...
func (s GaussianSampler[T]) normalFloat64() float64 {
	for {
		j := s.baseInt32Sampler.Sample() // Possibly negative
		i := j & 0x7F
		x := float64(j) * float64(wn[i])
		if uint32(num.Abs(j)) < kn[i] {
		...
		if i == 0 {
			...
			for {
				x = -math.Log(s.uniformFloat64()) * (1.0 / rn)
				y := -math.Log(s.uniformFloat64())
				if y+y >= x*x {
					break
				}
			}
		...
		if fn[i]+float32(s.uniformFloat64())*(fn[i-1]-fn[i]) < float32(math.Exp(-.5*x*x)) {
			return x
		}
	}
}
...
func (s GaussianSampler[T]) Sample() T {
	for {
		sample := s.normalFloat64() * s.StdDev

		if sample >= 0 && uint64(sample) <= num.MaxT[T]() {
			return T(sample)
		}
		if sample < 0 && uint64(-sample) <= num.MaxT[T]() {
			return T(sample)
