...
func (e *Encryptor[T]) UniEncryptPlaintextAssign(pt tfhe.GLWEPlaintext[T], ctOut UniEncryption[T]) {
	...
	for i := 0; i < ctOut.GadgetParameters.Level(); i++ {
		...
		e.SingleKeyEncryptor.PolyEvaluator.BinaryFourierPolyMulAddPolyAssign(ctOut.Value[0].Value[i].Value[1], e.buffer.auxFourierKey.Value[0], ctOut.Value[0].Value[i].Value[0])
...
func (e *Encryptor[T]) UniDecryptPlaintextAssign(ct UniEncryption[T], ptOut tfhe.GLWEPlaintext[T]) {
	...
	e.SingleKeyEncryptor.PolyEvaluator.BinaryFourierPolyMulSubPolyAssign(ct.Value[0].Value[ct.GadgetParameters.Level()-1].Value[1], e.buffer.auxFourierKey.Value[0], ptOut.Value)
}
