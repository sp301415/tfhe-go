...
func (e *GLWETransformer[T]) ToFourierUniEncryptionAssign(ctIn UniEncryption[T], ctOut FourierUniEncryption[T]) {
	for j := 0; j < ctIn.GadgetParameters.Level(); j++ {
		e.FourierEvaluator.ToScaledFourierPolyAssign(ctIn.Value[0].Value[j].Value[0], ctOut.Value[0].Value[j].Value[0])
		e.FourierEvaluator.ToScaledFourierPolyAssign(ctIn.Value[0].Value[j].Value[1], ctOut.Value[0].Value[j].Value[1])

		e.FourierEvaluator.ToScaledFourierPolyAssign(ctIn.Value[1].Value[j].Value[0], ctOut.Value[1].Value[j].Value[0])
		e.FourierEvaluator.ToScaledFourierPolyAssign(ctIn.Value[1].Value[j].Value[1], ctOut.Value[1].Value[j].Value[1])
	}
}
...
func (e *GLWETransformer[T]) ToUniEncryptionAssign(ctIn FourierUniEncryption[T], ctOut UniEncryption[T]) {
	for j := 0; j < ctIn.GadgetParameters.Level(); j++ {
		e.FourierEvaluator.ToScaledPolyAssign(ctIn.Value[0].Value[j].Value[0], ctOut.Value[0].Value[j].Value[0])
		e.FourierEvaluator.ToScaledPolyAssign(ctIn.Value[0].Value[j].Value[1], ctOut.Value[0].Value[j].Value[1])

		e.FourierEvaluator.ToScaledPolyAssign(ctIn.Value[1].Value[j].Value[0], ctOut.Value[1].Value[j].Value[0])
		e.FourierEvaluator.ToScaledPolyAssign(ctIn.Value[1].Value[j].Value[1], ctOut.Value[1].Value[j].Value[1])
	}
}
