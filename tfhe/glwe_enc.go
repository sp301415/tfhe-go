...
func (e Encryptor[T]) EncodeGLWE(messages []int) GLWEPlaintext[T] {
...
//	-If len(messages) < PolyDegree, the leftovers are padded with zero.
//	-If len(messages) > PolyDegree, the leftovers are discarded.
func (e Encryptor[T]) EncodeGLWECustom(messages []int, messageModulus, delta T) GLWEPlaintext[T] {
	pt := NewGLWEPlaintext(e.Parameters)
	for i := 0; i < e.Parameters.polyDegree && i < len(messages); i++ {
...
func (e Encryptor[T]) DecodeGLWE(pt GLWEPlaintext[T]) []int {
...
//
// If MessageModulus = 0, then no modulus reduction is performed.
func (e Encryptor[T]) DecodeGLWECustom(pt GLWEPlaintext[T], messageModulus, delta T) []int {
	messages := make([]int, e.Parameters.polyDegree)
	for i := 0; i < e.Parameters.polyDegree; i++ {
...
func (e Encryptor[T]) EncryptGGSWInPlace(pt GLWEPlaintext[T], ctOut GGSWCiphertext[T]) {
	...
	for i := 1; i < e.Parameters.glweDimension+1; i++ {
		...
		for j := 0; j < ctOut.decompParams.level; j++ {
			e.PolyEvaluator.ScalarMulInPlace(e.buffer.ggswPt, -ctOut.decompParams.ScaledBase(j), ctOut.Value[i].Value[j].Value[0])
