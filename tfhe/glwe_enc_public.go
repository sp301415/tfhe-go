...
func (e *PublicEncryptor[T]) EncryptGLWEBody(ct GLWECiphertext[T]) {
	...
	e.PolyEvaluator.BinaryFourierMulAddPolyAssign(e.PublicKey.GLWEKey.Value[0].Value[0], e.buffer.auxFourierKey.Value[0], ct.Value[0])
	for j := 1; j < e.Parameters.glweRank+1; j++ {
		e.PolyEvaluator.BinaryFourierMulAddPolyAssign(e.PublicKey.GLWEKey.Value[0].Value[j], e.buffer.auxFourierKey.Value[0], ct.Value[j])
	}
	for i := 1; i < e.Parameters.glweRank; i++ {
		for j := 0; j < e.Parameters.glweRank+1; j++ {
			e.PolyEvaluator.BinaryFourierMulAddPolyAssign(e.PublicKey.GLWEKey.Value[i].Value[j], e.buffer.auxFourierKey.Value[i], ct.Value[j])
		}
	}
