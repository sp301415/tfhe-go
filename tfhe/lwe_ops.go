package tfhe

import (
	"github.com/sp301415/tfhe/math/vec"
)

// AddLWE adds two LWE cipheretexts ct0, ct1 and returns the result.
func (e Evaluator[T]) AddLWE(ct0, ct1 LWECiphertext[T]) LWECiphertext[T] {
	ctOut := NewLWECiphertext(e.Parameters)
	e.AddLWEInPlace(ct0, ct1, ctOut)
	return ctOut
}

// AddLWEInPlace adds two LWE ciphertexts ct0, ct1 and writes to ctOut.
func (e Evaluator[T]) AddLWEInPlace(ct0, ct1, ctOut LWECiphertext[T]) {
	vec.AddInPlace(ct0.Value, ct1.Value, ctOut.Value)
}

// SubLWE subtracts ct0, ct1 and returns the result.
func (e Evaluator[T]) SubLWE(ct0, ct1 LWECiphertext[T]) LWECiphertext[T] {
	ctOut := NewLWECiphertext(e.Parameters)
	e.SubLWEInPlace(ct0, ct1, ctOut)
	return ctOut
}

// SubLWEInPlace subtracts ct0, ct1 and writes it to ctOut.
func (e Evaluator[T]) SubLWEInPlace(ct0, ct1, ctOut LWECiphertext[T]) {
	vec.SubInPlace(ct0.Value, ct1.Value, ctOut.Value)
}

// NegLWE negates ct0 and returns the result.
func (e Evaluator[T]) NegLWE(ct0 LWECiphertext[T]) LWECiphertext[T] {
	ctOut := NewLWECiphertext(e.Parameters)
	e.NegLWEInPlace(ct0, ctOut)
	return ctOut
}

// NegLWEInPlace negates ct0 and writes it to ctOut.
func (e Evaluator[T]) NegLWEInPlace(ct0, ctOut LWECiphertext[T]) {
	vec.NegInPlace(ct0.Value, ctOut.Value)
}

// ScalarAddLWE adds c to ct0 and returns the result.
func (e Evaluator[T]) ScalarAddLWE(ct0 LWECiphertext[T], c T) LWECiphertext[T] {
	ctOut := NewLWECiphertext(e.Parameters)
	e.ScalarAddLWEInPlace(ct0, c, ctOut)
	return ctOut
}

// ScalarMulAddInPlace adds c to ct0 and writes it to ctOut.
func (e Evaluator[T]) ScalarAddLWEInPlace(ct0 LWECiphertext[T], c T, ctOut LWECiphertext[T]) {
	ctOut.Value[0] += c * e.Parameters.delta
}

// ScalarMulLWE multplies c to ct0 and returns the result.
func (e Evaluator[T]) ScalarMulLWE(ct0 LWECiphertext[T], c T) LWECiphertext[T] {
	ctOut := NewLWECiphertext(e.Parameters)
	e.ScalarMulLWEInPlace(ct0, c, ctOut)
	return ctOut
}

// ScalarMulLWEInPlace multplies c to ct0 and writes it to ctOut.
func (e Evaluator[T]) ScalarMulLWEInPlace(ct0 LWECiphertext[T], c T, ctOut LWECiphertext[T]) {
	vec.ScalarMulInPlace(ct0.Value, c, ctOut.Value)
}

// ScalarMulAddLWEInPlace multiplies c to ct1 and adds to ctOut.
func (e Evaluator[T]) ScalarMulAddLWEInPlace(ct0 LWECiphertext[T], c T, ctOut LWECiphertext[T]) {
	vec.ScalarMulAddInPlace(ct0.Value, c, ctOut.Value)
}

// ScalarMulSubLWEInPlace multiplies c to ct0 and subtracts from ctOut.
func (e Evaluator[T]) ScalarMulSubLWEInPlace(ct0 LWECiphertext[T], c T, ctOut LWECiphertext[T]) {
	vec.ScalarMulSubInPlace(ct0.Value, c, ctOut.Value)
}
