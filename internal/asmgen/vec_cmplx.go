...
func elementWiseMulCmplxAssignAVX2() {
	...
	x0Real, x0Imag := YMM(), YMM()
	VMOVUPD(Mem{Base: v0}, x0Real)
	VMOVUPD(Mem{Base: v0, Disp: 32}, x0Imag)
	x1Real, x1Imag := YMM(), YMM()
	VMOVUPD(Mem{Base: v1}, x1Real)
	VMOVUPD(Mem{Base: v1, Disp: 32}, x1Imag)
	...
	VMOVUPD(xOutReal, Mem{Base: vOut})
	VMOVUPD(xOutImag, Mem{Base: vOut, Disp: 32})
...
func elementWiseMulAddCmplxAssignAVX2() {
	...
	VMOVUPD(Mem{Base: v1, Disp: 32}, x1Imag)
	xOutReal, xOutImag := YMM(), YMM()
	VMOVUPD(Mem{Base: vOut}, xOutReal)
	VMOVUPD(Mem{Base: vOut, Disp: 32}, xOutImag)
